...
STEP 4 - Sanity check. Read the manifest. Confirm product.images has at least one URL, product.price is present, and klaviyo.campaign_url exists. Note for the report: product.thumbnails (0-3 extra images shown), the number of product.specs rows (0 means the description had no bullet list, so the SPECIFICATIONS box was omitted), and anything odd (a blurb that looks like a spec line, a headline that looks wrong). If product.live is false the product is under embargo (draft or not yet published to the Online Store): that is allowed and expected - the e-mail is prepared ahead so Igor can schedule it in Klaviyo - but the report MUST start its second line with "NOT LIVE YET: publish the product in Shopify before the scheduled send, otherwise the links 404."
